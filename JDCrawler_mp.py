...
def get_statics(driver):
	driver.find_element_by_css_selector('#pingjia > p').click()
	time.sleep(2)
	total = driver.find_element_by_css_selector('.tab-assess-rednum').text
	good = driver.find_element_by_css_selector('#tabs > li:nth-child(2) > p.tab-assess-num').text
	neutral = driver.find_element_by_css_selector('#tabs > li:nth-child(3) > p.tab-assess-num').text
	bad = driver.find_element_by_css_selector('#tabs > li:nth-child(4) > p.tab-assess-num').text
	pic = driver.find_element_by_css_selector('#tabs > li:nth-child(5) > p.tab-assess-num').text
	return total, good, neutral, bad, pic
...
def insert_2_db(url, total, good, neutral, bad, pic):
	try:
		client = MongoClient('192.168.11.153',27018)
		db = client['RDU_JD']
		col_phone = db['statics']
		col_phone.insert({"url":url, "total":total, "good":good, "neutral":neutral, "bad":bad, "pic":pic, "timestamp":datetime.now()})
		client.close()
	except:
		time.sleep(10)
		client = MongoClient('192.168.11.153',27018)
...
if __name__ == '__main__':
	...
	#url = 'http://item.m.jd.com/product/3311953.html?sid=4f4a9e41bda015097ae6d0f37ac1f21f'
		if check_permute(url):
			...
			print url
		#normal url 2 mobile url
		url_mobile = "http://item.m.jd.com/product/" + url.split('/')[-1].split('.')[0] + ".html"
		print url_mobile
		driver.get(url_mobile)
		driver.maximize_window()

		total, good, neutral, bad, pic = get_statics(driver)
		print 'total:', str(total), ' good:', str(good), ' neutral:', str(neutral), ' bad:',str(bad), ' pic:',str(pic)
		insert_2_db(url, total, good, neutral, bad, pic)
